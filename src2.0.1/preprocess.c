...
void print_preprocess_usage()
{
	...
	printf(" preprocess 1SBB.pdb -u no -d 1    #consider dynamic using GNM model\n");
...
int main(int argc, char *argv[])
{
	...
	for(arg_i=2;arg_i<argc;arg_i+=2){
		if(argv[arg_i][0]!='-'||(argv[arg_i][1]!='c'&&argv[arg_i][1]!='o'&&argv[arg_i][1]!='d'&&argv[arg_i][1]!='u'&&argv[arg_i][1]!='s'&&argv[arg_i][1]!='a')){
			printf("\nERROR at '%s': should be one of these options: -c -o -d -u -s -a!\n\n",argv[arg_i]);
			print_preprocess_usage();
			return EXIT_SUCCESS;
		}

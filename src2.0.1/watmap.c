...
int main(int argc, char *argv[])
{
	...
	decide_gridsize(&pro,GRIDSIZE, &GRID3SIZE);
	snew(mattergrid, GRID3SIZE);
	init_mattergrid(mattergrid, GRID3SIZE);
	gen_mattergrid(&pro, mattergrid,GRIDSIZE);
	printf("generate mattergrid\n");

	snew(HBgrid, GRID3SIZE);
	init_HBgrid(HBgrid, GRID3SIZE);
	gen_HBgrid(&pro, HBgrid,GRIDSIZE,GRID3SIZE);
	printf("generate HBgrid\n");

	snew(pocketgrid, GRID3SIZE);
	init_pocketgrid(mattergrid,pocketgrid, GRID3SIZE);
	gen_pocketgrid(&pro, pocketgrid,mattergrid,GRIDSIZE,GRID3SIZE);
	printf("generate pocketgrid\n");

	snew(watenegrid, GRID3SIZE);
	init_watenegrid(watenegrid, GRID3SIZE);
	gen_watenegrid(watenegrid,mattergrid, pocketgrid,HBgrid,GRIDSIZE,GRID3SIZE);
	printf("generate watenegrid\n");

	snew(enewatxyz, pro.surfatN*10);
	enewatn=enegrid2watpositin(mattergrid, pocketgrid,watenegrid,HBgrid,enewatxyz,0,1,GRIDSIZE,GRID3SIZE);
	enewatn+=enegrid2watpositin(mattergrid, pocketgrid,watenegrid,HBgrid,enewatxyz,enewatn,-1,GRIDSIZE,GRID3SIZE);
	sfree(watenegrid);

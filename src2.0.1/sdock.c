...
void calFFTmult(fftwf_complex *fg1,fftwf_complex *fg2,fftwf_complex *fg3, int GRID3SIZE)
{
    int i;
...
void calFFTmult2(fftwf_complex *fg1,fftwf_complex *fg2,fftwf_complex *fg3, int GRID3SIZE)
{
    int i;
...
void inittot(int GRIDSIZE[3], int GRID3SIZE)
{
	int i,j,k,g1;
	int ii,jj,kk,g2;
	...
	for(i=0;i<GRIDSIZE[0];i++){
		...
		for(j=0;j<GRIDSIZE[1];j++){
			...
			for(k=0;k<GRIDSIZE[2];k++){
				...
				g1=getgindex(i,j,k,GRIDSIZE);
				g2=getgindex(ii,jj,kk,GRIDSIZE);
				totalscore[g2]=repscore[g1]*REPWEIGHT/(GRID3SIZE);
			}
		}
	}
}
void inverseFFT(int wmod, int GRIDSIZE[3], int GRID3SIZE,fftwf_complex	*Rrepgrid2, fftwf_complex *Relegrid,fftwf_complex *Rvdwgrid,fftwf_complex *Rsolgrid1,fftwf_complex *Rsolgrid2,fftwf_complex *Rsolgrid3,fftwf_complex *Rwatgrid,fftwf_complex *Rindgrid,fftwf_complex *RNhbgrid,fftwf_complex *ROhbgrid, fftwf_complex *Lrepgrid2,fftwf_complex *Lelegrid,fftwf_complex *Lvdwgrid,fftwf_complex *Lsolgrid1,fftwf_complex *Lsolgrid2,fftwf_complex *Lsolgrid3,fftwf_complex *Lwatgrid,fftwf_complex *Lindgrid,fftwf_complex *LNhbgrid,fftwf_complex *LOhbgrid)
{
	int i;
      float tden;
	...
	calFFTmult2(Rrepgrid2,Lrepgrid2,ffttmp,GRID3SIZE);
	fftwf_execute(p_ffttmp1) ;
	inittot(GRIDSIZE, GRID3SIZE);
	for(i=0;i<GRID3SIZE;i++)
		repscore[i]=totalscore[i]/REPWEIGHT;
	calFFTmult(Rvdwgrid,Lvdwgrid,ffttmp,GRID3SIZE);
	fftwf_execute(p_ffttmp);
	for(i=0;i<GRID3SIZE;i++){
		vdwscore[i]=ffttmp[i][1]/(GRID3SIZE)/2.0;
		totalscore[i]+=vdwscore[i];
	}
	calFFTmult(Relegrid,Lelegrid,ffttmp,GRID3SIZE);
	fftwf_execute(p_ffttmp);
	for(i=0;i<GRID3SIZE;i++){
		elescore[i]=ffttmp[i][1]/(GRID3SIZE)/2.0;
		totalscore[i]+=elescore[i]*ELEWEIGHT;
	}
	calFFTmult(Rsolgrid1,Lsolgrid1,ffttmp,GRID3SIZE);
	fftwf_execute(p_ffttmp);
	for(i=0;i<GRID3SIZE;i++){
		solscore[i]=ffttmp[i][1]/(GRID3SIZE);
	}
	calFFTmult(Rsolgrid2,Lsolgrid2,ffttmp,GRID3SIZE);
	fftwf_execute(p_ffttmp);
	for(i=0;i<GRID3SIZE;i++){
		solscore[i]+=ffttmp[i][1]/(GRID3SIZE);
	}
	calFFTmult(Rsolgrid3,Lsolgrid3,ffttmp,GRID3SIZE);
	fftwf_execute(p_ffttmp);
	for(i=0;i<GRID3SIZE;i++){
		solscore[i]+=ffttmp[i][1]/(GRID3SIZE);
	...
	if(wmod==WATMODE) {
		calFFTmult(Rwatgrid,Lwatgrid,ffttmp,GRID3SIZE);
		fftwf_execute(p_ffttmp);
		for(i=0;i<GRID3SIZE;i++){
	...
	calFFTmult(Rindgrid,Lindgrid,ffttmp,GRID3SIZE);
	fftwf_execute(p_ffttmp);
	for(i=0;i<GRID3SIZE;i++){
		indscore[i]=ffttmp[i][1]/(GRID3SIZE);
		totalscore[i]+=indscore[i]*INDWEIGHT;
	}
	calFFTmult(RNhbgrid,LNhbgrid,ffttmp,GRID3SIZE);
	fftwf_execute(p_ffttmp);
	for(i=0;i<GRID3SIZE;i++){
		Nhbscore[i]=ffttmp[i][1]/(GRID3SIZE);
		totalscore[i]+=Nhbscore[i]*HBNWEIGHT;
	}
	calFFTmult(ROhbgrid,LOhbgrid,ffttmp,GRID3SIZE);
	fftwf_execute(p_ffttmp);
	for(i=0;i<GRID3SIZE;i++){
...
void record_best(int roti, int GRIDSIZE[3],DOCKRECORD *dock_result,int *count, int RESULTKEEPN)
{
	int x,y,z,xyz;
	int fx,fy,fz;
	int N=*count;
		...
		for( y = 0 ; y < GRIDSIZE[1] ; y ++ ) {
			...
			for( z = 0 ; z < GRIDSIZE[2] ; z ++ ) {
				...
				if(totalscore[xyz]<KEEPSCORE){
					N=save_dock(roti, fx*GRIDSPACESIZE,fy*GRIDSPACESIZE,fz*GRIDSPACESIZE,totalscore[xyz],repscore[xyz],vdwscore[xyz],elescore[xyz],solscore[xyz],watscore[xyz],indscore[xyz],Nhbscore[xyz],Ohbscore[xyz], dock_result,N, RESULTKEEPN);
				}
			}
		}
	}
...
void Dock(STRUCTURE *ligand,SURFWAT *ligwat,int ROTN, float (*ROTPLAN)[4],float prog_per,int wmod,int GRIDSIZE[3],int GRID3SIZE,int *mattergrid,fftwf_complex *Rrepgrid2,fftwf_complex *Relegrid,fftwf_complex *Rvdwgrid,fftwf_complex *Rsolgrid1,fftwf_complex *Rsolgrid2,fftwf_complex *Rsolgrid3,fftwf_complex *Rwatgrid,fftwf_complex *Rindgrid,fftwf_complex *RNhbgrid,fftwf_complex *ROhbgrid, float *Lrepgrid,fftwf_complex *Lrepgrid2,fftwf_complex *Lelegrid,fftwf_complex *Lvdwgrid,fftwf_complex *Lsolgrid1,fftwf_complex *Lsolgrid2,fftwf_complex *Lsolgrid3,fftwf_complex *Lwatgrid,fftwf_complex *Lindgrid,fftwf_complex *LNhbgrid,fftwf_complex *LOhbgrid,DOCKRECORD *dock_result,int RESULTKEEPN)
{
	int ri;
	float m[3][3];
	...
	for(ri=0;ri<ROTN;ri++){
		...
		gen_ligand_grid(ligand,ligwat,wmod,GRIDSIZE,GRID3SIZE,mattergrid,Lrepgrid,Lelegrid,Lvdwgrid,Lsolgrid1,Lsolgrid2,Lsolgrid3,Lwatgrid,Lindgrid,LNhbgrid,LOhbgrid);
		ligandFFT(wmod);
		inverseFFT(wmod,GRIDSIZE, GRID3SIZE, Rrepgrid2, Relegrid,Rvdwgrid,Rsolgrid1,Rsolgrid2,Rsolgrid3,Rwatgrid,Rindgrid,RNhbgrid,ROhbgrid,Lrepgrid2,Lelegrid,Lvdwgrid,Lsolgrid1,Lsolgrid2,Lsolgrid3,Lwatgrid,Lindgrid,LNhbgrid,LOhbgrid);
		record_best(ri,GRIDSIZE,dock_result,&count, RESULTKEEPN);
	}
}
...
int main(int argc, char *argv[])
{
	...
	int GRIDSIZE[3];
	int GRID3SIZE;
	fftwf_complex *Rrepgrid2=NULL,*Relegrid=NULL,*Rvdwgrid=NULL,*Rsolgrid1=NULL,*Rsolgrid2=NULL,*Rsolgrid3=NULL,*Rwatgrid=NULL,*Rindgrid=NULL,*RNhbgrid=NULL,*ROhbgrid=NULL;
	fftwf_complex *Lrepgrid2=NULL,*Lelegrid=NULL,*Lvdwgrid=NULL,*Lsolgrid1=NULL,*Lsolgrid2=NULL,*Lsolgrid3=NULL,*Lwatgrid=NULL,*Lindgrid=NULL,*LNhbgrid=NULL,*LOhbgrid=NULL;
	float	*Rrepgrid=NULL,*Lrepgrid=NULL;
	int *mattergrid=NULL;
	...
	t=decide_gridsize(&proA,&proB,GRIDSIZE,&GRID3SIZE);
	if(t==0) {
		receptor = &proA;
		ligand = &proB;
		fixpro(receptor);
	...
	if(dockmod==WATMODE) fftnew(Rwatgrid,GRID3SIZE);
	...
	if(dockmod==WATMODE) fftnew(Lwatgrid,GRID3SIZE);
	...
	if(dockmod==WATMODE) p_Rwatgrid=fftwf_plan_dft_3d(GRIDSIZE[0],GRIDSIZE[1],GRIDSIZE[2],Rwatgrid,Rwatgrid,FFTW_BACKWARD,FFTW_ESTIMATE);
	...
	if(dockmod==WATMODE) p_Lwatgrid=fftwf_plan_dft_3d(GRIDSIZE[0],GRIDSIZE[1],GRIDSIZE[2],Lwatgrid,Lwatgrid,FFTW_FORWARD,FFTW_MEASURE);
	...
	gen_receptor_grid(receptor,recwat,dockmod,GRIDSIZE,GRID3SIZE, mattergrid, Rrepgrid, Relegrid,Rvdwgrid,Rsolgrid1,Rsolgrid2,Rsolgrid3,Rwatgrid,Rindgrid,RNhbgrid,ROhbgrid);
	receptorFFT(dockmod);
	Dock(ligand,ligwat,ROTN,ROTPLAN,prog_per,dockmod,GRIDSIZE,GRID3SIZE,mattergrid,Rrepgrid2, Relegrid,Rvdwgrid,Rsolgrid1,Rsolgrid2,Rsolgrid3,Rwatgrid,Rindgrid,RNhbgrid,ROhbgrid,Lrepgrid,Lrepgrid2,Lelegrid,Lvdwgrid,Lsolgrid1,Lsolgrid2,Lsolgrid3,Lwatgrid,Lindgrid,LNhbgrid,LOhbgrid,dock_result, RESULTKEEPN);
 	...
 	fftwf_destroy_plan( p_Rsolgrid1 ) ;
  	...
  	if(dockmod==WATMODE) fftwf_destroy_plan( p_Rwatgrid) ;
 	...
 	fftwf_destroy_plan( p_Lsolgrid1 ) ;
  	...
  	if(dockmod==WATMODE) fftwf_destroy_plan( p_Lwatgrid) ;
	...
	fftfree(Rsolgrid2);
	fftfree(Rsolgrid3);
	if(dockmod==WATMODE) fftfree(Rwatgrid);
	fftfree(Rindgrid);
	fftfree(RNhbgrid);
	fftfree(ROhbgrid);
	fftfree(Lrepgrid);
	...
	fftfree(Lsolgrid2);
	fftfree(Lsolgrid3);
	if(dockmod==WATMODE) fftfree(Lwatgrid);
	fftfree(Lindgrid);
	fftfree(LNhbgrid);
	fftfree(LOhbgrid);
	fftfree(repscore);
	...
	arrange_record(result_index,RESULTKEEPN);
	clusterN=cluster_result(receptorcaN,receptorca,ligandcaN,ligandca, max_clusterN, RESULTKEEPN,cluster,dock_result,result_index,LRMSDCUTOFF,ROTPLAN);

	if(arg_result==0&&arg_rotfile==0)
		print_cluster(dockmod,argv[1],argv[2],t,"sdock_record","so3layer.qua",REPWEIGHT,ELEWEIGHT,SOLWEIGHT,WATWEIGHT,INDWEIGHT,HBNWEIGHT,HBOWEIGHT,clusterN, cluster,dock_result,result_index,LRMSDCUTOFF);
	else if(arg_result==0&&arg_rotfile!=0)
		print_cluster(dockmod,argv[1],argv[2],t,"sdock_record",argv[arg_rotfile],REPWEIGHT,ELEWEIGHT,SOLWEIGHT,WATWEIGHT,INDWEIGHT,HBNWEIGHT,HBOWEIGHT,clusterN, cluster,dock_result,result_index,LRMSDCUTOFF);
	else if(arg_result!=0&&arg_rotfile==0)
		print_cluster(dockmod,argv[1],argv[2],t,argv[arg_result],"so3layer.qua",REPWEIGHT,ELEWEIGHT,SOLWEIGHT,WATWEIGHT,INDWEIGHT,HBNWEIGHT,HBOWEIGHT,clusterN, cluster,dock_result,result_index,LRMSDCUTOFF);
	else
		print_cluster(dockmod,argv[1],argv[2],t,argv[arg_result],argv[arg_rotfile],REPWEIGHT,ELEWEIGHT,SOLWEIGHT,WATWEIGHT,INDWEIGHT,HBNWEIGHT,HBOWEIGHT,clusterN, cluster,dock_result,result_index,LRMSDCUTOFF);

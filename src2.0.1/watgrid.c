...
void gen_mattergrid(STRUCTURE *pro, int *matbulk,int GRIDSIZE[3])
{
	int ia;
	int i,j,k;
	float rad,rad2;
...
void gen_HBgrid(STRUCTURE *pro, float *HBgrid,int GRIDSIZE[3], int GRID3SIZE)
{
	int ia;
	int i,j,k;
...
void decide_gridsize(STRUCTURE *p1,int GRIDSIZE[3], int *GRID3SIZE)
{
	float span1[3];
	int GS1[3];
	int i;
...
int inpocket(int a,int b,float m,int GRIDSIZE[3])
{
	int ai,aj,ak;
	int bi,bj,bk;
	int x,y,z,d;
...
void getpocket(STRUCTURE *pro, int *mattergrid, int *grid, float RAD,int GRIDSIZE[3], int GRID3SIZE)
{
	int ia;
	int i,j,k;
	int ii,jj,kk,gg;
	...
	for(i=0;i<GRIDSIZE[0];i++)for(j=0;j<GRIDSIZE[1];j++)for(k=0;k<GRIDSIZE[2];k++){
		...
		if(grid[g]==EDGEPOINT){
			...
			for(kk=BSMAX(0,k-GSN);kk<=BSMIN(GRIDSIZE[2]-1,k+GSN);kk++){
				gg=getgindex(ii,jj,kk,GRIDSIZE);
...
void gen_pocketgrid(STRUCTURE *pro,int *pocketgrid, int *mattergrid,int GRIDSIZE[3], int GRID3SIZE)
{
	int *grid;
	int i;
	int ia;
...
float smoothenemap(float *enegrid,int *mattergrid, int *pocketgrid, int i,int j,int k,int neiN,int neighbor[][3],int GRIDSIZE[3])
{
	int g;
	int ni;
	float pmax,nmax;
	...
	for(ni=0;ni<neiN;ni++){
		g=getgindex(i+neighbor[ni][0],j+neighbor[ni][1],k+neighbor[ni][2],GRIDSIZE);
		if(mattergrid[g]==PROTEINSPACE) continue;
...
void gen_watenegrid(float *watenegrid, int *mattergrid, int *pocketgrid, float *HBgrid,int GRIDSIZE[3], int GRID3SIZE)
{
	int g;
	int i,j,k;
	float *enegrid;
	...
	for(k=0;k<GRIDSIZE[2];k++){
		...
		if(mattergrid[g]==PROTEINSPACE) continue;
		if(pocketgrid[g]==WATBULKGEO) continue;
		watenegrid[g]=smoothenemap(enegrid,mattergrid,pocketgrid,i,j,k,neiN,neighbor, GRIDSIZE);
	}
	sfree(enegrid);
}
...
int enegrid2watpositin(int *mattergrid, int *pocketgrid,float *enegrid, float *HBgrid,float enewatxyz[][4], int start, int type,int GRIDSIZE[3], int GRID3SIZE)
{
	int pn;
	float (*p)[3];
	float *ene;

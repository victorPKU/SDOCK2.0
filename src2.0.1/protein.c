...
int read_atm(char *atmparaf, PROTEINAT atdata[800])
{
	FILE *atmf;
...
int lookupATMtab(char *aname,char *rname,int atdataN, PROTEINAT atdata[800])
{
	int i;
...
void get_atom_para(PROTEIN *pro, char *fn)
{
	...
	atdataN=read_atm(fn, atdata);
	for(ia=0;ia<pro->atmN;ia++){
		index=lookupATMtab(pro->a[ia].abbrname,pro->a[ia].name+4,atdataN,atdata);
		if(index==-1){
